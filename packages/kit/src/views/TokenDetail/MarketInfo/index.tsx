...
const Header: FC<Props> = ({ token }) => {
  ...
  if (token && token.isNative) {
    const priceConfigs = serverPresetNetworks.find(
      (item) => item.chainId === token.chainId,
    )?.priceConfigs;
    const coingeckoChannel = priceConfigs?.find(
      (item) => item.channel === 'coingecko',
    );
    const coingeckoId = coingeckoChannel?.native;
    if (coingeckoId) {
