...
import { updateFaviconUserBrowserHistory } from '../../../../store/reducers/discover';
import {
  homeResettingFlags,
  homeTab,
  setWebTabData,
...
export const onNavigation: OnWebviewNavigation = ({
  ...
  if (url && typeof url === 'string') {
    backgroundApiProxy.dispatch(
      updateFaviconUserBrowserHistory({
        url,
        logoUrl: favicon ?? curTab?.favicon,
      }),
    );
  }
  ...
  backgroundApiProxy.dispatch(
    setWebTabData({
      id: curId,
      title,
      favicon: favicon ?? curTab?.favicon,
      canGoBack,
      canGoForward,
      loading,
    }),
