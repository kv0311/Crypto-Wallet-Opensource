...
export default class ServiceSwap extends ServiceBase {
  ...
  async setDefaultInputToken() {
    ...
    const USDT = {
      ...
      decimals: 18,
      logoURI:
        'https://common.onekey-asset.com/token/evm-56/0x55d398326f99059fF775485246999027B3197955.jpg',
      impl: 'evm',
      chainId: '56',
    } as ServerToken;
    ...
    const CAMLY = {
      ...
      decimals: 3,
      logoURI: 'https://camly.co/images/camly-coin-1.png',
      impl: 'evm',
      chainId: '56',
    } as ServerToken;

    // const network = await engine.getNetwork(OnekeyNetwork.bsc);
    ...
    // }
    // if (!inputToken) {
    const tokenIn = formatServerToken(USDT);
    this.setInputToken(tokenIn);
    // }
    // const outputToken = appSelector((s) => s.swap.outputToken);
    // if (!outputToken) {
    const tokenOut = formatServerToken(CAMLY);
    this.setOutputToken({ ...tokenOut, coingeckoId: '' });
    // }
  }
