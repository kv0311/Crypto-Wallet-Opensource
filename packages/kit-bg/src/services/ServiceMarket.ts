...
export default class ServiceMarket extends ServiceBase {
  ...
  async fetchMarketCategorys() {
    ...
    dispatch(
      saveMarketCategorys(datas.filter((d) => d.categoryId !== 'trending')),
    );
  ...
  async fetchMarketList({
    ...
    const data = await fetchData<MarketTokenItem[]>(
      ...
      {
        category:
          categoryId === MARKET_TRENDING_CATEGORYID ? undefined : categoryId,
    ...
    const dataNew = data.map((t) => ({
      ...t,
      logoURI: t.image,
    }));
    dispatch(updateMarketTokens({ categoryId, marketTokens: dataNew }));
